...
Each **seat** is one team. Under each seat you say what that team owns, who it serves, and what makes it want a request or pass it on. When you dispatch a request, every team takes a position by its own charter, and you get to see where they agree (usually who owns it) and where they don't (usually the plan).

Copy the block below for each team. Fill it in yourself, or ask the skill to help from a team card.

---

## Copy this for each team
...
<!-- Two teams to start. Add at least one MORE that wants something different. -->
...
Format bias: a durable artifact that lives on Learn and scales.
Voice: Thinks in shelf-life and reach; asks "who maintains it?"
...
They say NO / route elsewhere when: the real need is evergreen self-serve; there's nothing to deliver yet.
Format bias: a live thing an instructor runs.
Voice: Talks in calendars and capacity; opens with "who's the audience, and when do they need to be ready?"
...
| Setting | Value | Notes |
|---|---|---|
| The decision has five parts | owner · audience · plan · disposition · next-action | *owner* is who fields it; *plan* is the deliverables (build once, reuse across teams). |
| Dispositions | proceed-as-is · reshape · split · defer · decline-and-redirect | `proceed-as-is` = route it as asked; `split` = it's genuinely two deliverables; `defer`/`decline-and-redirect` = for a request that isn't ready. |
| When a request is under-specified | missing audience, topic, or outcome → **sharpen first**, don't route | A rough idea gets `defer` or `decline-and-redirect`, not a confident route. |
| Build once, reuse | if two teams need the same thing, the plan says *build once, reused by* | Never the same deliverable built twice. |
