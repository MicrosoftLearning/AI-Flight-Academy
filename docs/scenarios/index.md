---
title: Scenarios
---

# Scenarios

Each scenario is a **real, relatable Microsoft work problem** on safe, sanitized data. Every scenario runs on every [track](/levels/), so pick the one you want to solve.

::: warning Still being finalized
Scenario 3 is still being finalized with the leadership team and business managers. Scenarios 1 and 2 are drafted and in active refinement.
:::

- 🚧 [Scenario 1 – The Digital Twin](/scenarios/scenario-1) – **work in progress**
- 🚧 [Scenario 2 – The Greenlight](/scenarios/scenario-2) – **work in progress**
- [Scenario 3 (TBD)](/scenarios/scenario-3)
